  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2 uppercase tracking-wide">Audit Infrastructure</h1>
          ...
          <table className="min-w-full divide-y divide-slate-100 text-left">
            <thead className="bg-white">
              <tr>
                ...
                <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">IP Address</th>
              ...
              {loading ? (
                ...
                filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="h-7 w-7 rounded-lg bg-slate-100 flex items-center justify-center mr-3 border border-slate-200">
                          <span className="text-[10px] font-bold text-slate-600">{(log.user_email || 'SYS')[0].toUpperCase()}</span>
                        </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 text-[10px] font-bold uppercase rounded border ${getSeverityColor(log.severity)}`}>
                        {log.severity}
                      </span>
                    </td>
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 bg-slate-900 rounded-2xl border border-slate-800 shadow-xl">
          <span className="block text-[10px] font-bold text-indigo-400 uppercase tracking-wider mb-4">Integrity Verification</span>
        ...
        <div className="p-6 bg-white rounded-2xl border border-slate-200 shadow-lg">
          <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-4">Retention Policy</span>
          <p className="text-sm text-slate-600 leading-relaxed font-medium">
            System activity data is retained for 365 days in accordance with the security compliance protocol.
          </p>
        </div>
        <div className="p-6 bg-white rounded-2xl border border-slate-200 shadow-lg">
          <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-4">Anomaly Detection</span>
          <p className="text-sm text-slate-600 leading-relaxed font-medium">
